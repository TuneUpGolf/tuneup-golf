...
class PurchaseController extends Controller
{
    ...
    {
        $purchase = Purchase::find($request->query('purchase_id'));
        if (!!$purchase && $purchase->lesson->type == Lesson::LESSON_TYPE_INPERSON) $slot = Slots::find($purchase?->slot_id);
        try {
            if (!empty($purchase)) {
            ...
            if ($purchase?->lesson->lesson_quantity > $purchase->lessons_used) {
                try {
                    ...
                    if ($purchase->status === Purchase::STATUS_COMPLETE) {
                        ...
                        $message = __('Hello, :name, a video has been submited by :studentName against your lesson ', [
                            'name' => $purchase->lesson->user->name,
                            'studentName' => $purchase->student->name,
                            'lessonName'  => $purchase->lesson->lesson_name,
                        ]);
            ...
            if (Auth::user()->type == Role::ROLE_INSTRUCTOR) {
                ...
                if (isset($purchase)) {
                    ...
                    if (isset($purchaseVideo)) {
                        ...
                        $message = __('Hello, :name, you just recieved feedback on :lessonName by :instructorName on one of your submissions', [
                            'name' => $purchase->student->name,
                            'instructorName' => $purchase->lesson->user->name,
                            'lessonName'  => $purchase->lesson->lesson_name,
                        ]);
    ...
    public function getAll()
    {
        try {
            if (Auth::user()->can('manage-purchases')) {
                if (Auth::user()->active_status == true) {
                    if (Auth::user()->type == Role::ROLE_INSTRUCTOR) {
                        $purchases = Purchase::where('instructor_id', Auth::user()->id);
                        request()->student_request = true;
                    } else if (Auth::user()->type == Role::ROLE_STUDENT) {
                        $purchases = Purchase::where('student_id', Auth::user()->id);
                    }
    ...
    public function addFeedBack(Request $request)
    {
        ...
        try {
            if (Auth::user()->can('manage-purchases') && $purchaseVideo = PurchaseVideos::find($request->purchase_video_id)) {
                ...
                $message = __('Hello, :name, you just recieved feedback on :lessonName by :instructorName on one of your submissions', [
                    'name' => $purchaseVideo->purchase->student->name,
                    'instructorName' => $purchaseVideo->purchase->lesson->user->name,
                    'lessonName'  => $purchaseVideo->purchase->lesson->lesson_name,
                ]);
