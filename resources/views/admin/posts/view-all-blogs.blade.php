                                        ...
                                        <div class="article-card-inner  d-flex align-items-center">
                                            <div class="card-image">
                                                <a href="{{ route('view.blog', $recentBlog->slug) }}" tabindex="0">
                                                    <img src="{{ isset($recentBlog->photo) ? Storage::url(tenant('id') . '/' . $recentBlog->photo) : asset('assets/images/landingpage-2-image/blog-card-img.png') }}"
                                    ...
                                    <div class="article-card-inner">
                                        <div class="article-card-image">
                                            <a href="{{ route('view.blog', $lastBlog->slug) }}">
                                                <img src="{{ isset($lastBlog->photo) ? Storage::url(tenant('id') . '/' . $lastBlog->photo) : asset('assets/images/landingpage-2-image/blog-card-img.png') }}"
                                                    alt="blog-card-image">
                                            </a>
                                        </div>
                                    ...
                                    <div class="article-card-inner">
                                        <div class="article-card-image">
                                            <a href="{{ route('view.blog', $blog->slug) }}">
                                                <img src="{{ isset($blog->photo) ? Storage::url(tenant('id') . '/' . $blog->photo) : asset('assets/images/landingpage-2-image/blog-card-img-2.png') }}"
                                                    alt="blog-card-image">
                                            </a>
                                        </div>
                                        <div class="article-card-content">
            ...
            $('body').on('change', '#search-category', function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        var allBlogs = response.all_blogs.data;
                        var appendedContent = '';
                        $('.append-blog-data').empty();
                        var storageBaseUrl = "{{ Storage::url(tenant('id') . '/') }}";
                        var baseUrl = "{{ url('/') }}";
                        $.each(allBlogs, function(index, blog) {
                            var imageUrl = blog.photo ? storageBaseUrl + blog.photo :
                                'assets/images/landingpage-2-image/blog-card-img-2.png';
                            var redirectUrl = baseUrl + "/blogs/" + blog.slug;
                            var createdAt = blog.created_at;
                            var blogHtml = `<div class="article-card">
                                                <div class="article-card-inner">
                                                    <div class="article-card-image">
                                                        <a href="${redirectUrl}">
                                                    ...
                                                    <div class="article-card-content">
                                                        ...
                                                        <p>${blog.short_description ? blog.short_description : 'A step-by-step guide on how to configure and implement multi-tenancy in a Laravel application, including tenant isolation and database separation.'}</p>
                                                    </div>
                                                </div>
                                            </div>`;
                            // Append the new blog entry HTML to the existing content
                            appendedContent += blogHtml;
                        });
                        $('.append-blog-data').append(appendedContent);
                    }
                });
            });
        });
