...
@if ($document->document_menu && $document->status == 1)
    <a class="btn btn-success btn-sm copy_menu action-btn-fix" onclick="copyToClipboard('#copy-menu-{{ $document->id }}')"
    ...
    <a href="{{ route('document.public', $document->document_menu->slug) }}" data-bs-toggle="tooltip"
        ...
        class="btn btn-info mr-1 btn-sm action-btn-fix" data-toggle="tooltip"><i class="ti ti-eye"></i></a>
@endif
@if ($document->status != 1)
    <a class="btn btn-sm small btn btn-success action-btn-fix" href="{{ route('document.status', $document->id) }}"
        data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-original-title="{{ __('Active') }}">
        <i class="ti ti-checks text-white"></i>
    </a>
@else
    <a class="btn btn-sm small btn btn-danger action-btn-fix" href="{{ route('document.status', $document->id) }}"
        data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-original-title="{{ __('Deactive') }}">
        <i class="ti ti-ban text-white"></i>
    </a>
@endif
<a class="btn btn-sm small btn btn-info edit_menu cust_btn action-btn-fix" data-bs-toggle="tooltip" data-bs-placement="bottom"
    data-bs-original-title="{{ __('Design document') }}" href="{{ route('document.design', $document->id) }}"
    id="edit-menu">
    <i class="ti ti-brush"></i>
</a>
@can('edit-document')
    <a class="btn btn-sm small btn btn-warning action-btn-fix" href="{{ route('document.edit', $document->id) }}" data-bs-toggle="tooltip"
