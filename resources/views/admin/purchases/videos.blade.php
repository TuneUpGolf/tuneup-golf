...
@section('content')
<div class="flex items-start justify-between border-b border-gray-400 pb-4 mb-5">
    <div class="max-w-lg">
...
<div class="flex flex-col md:flex-row gap-6 bg-white p-4 rounded-lg">
    ...
    {{-- Left: Video Section --}}
    <div class="video-section-col flex flex-col gap-6 w-full md:w-3/4">
        @if($purchaseVideoUrl)
            <div class="video-wrap">
                <video controls autoplay loop muted src="{{ $purchaseVideoUrl }}" class="w-full md:w-80 h-60 rounded-lg"></video>
        ...
        @if($purchaseVideo2Url)
            <div class="video-wrap">
                <video controls autoplay loop muted src="{{ $purchaseVideo2Url }}" class="w-full md:w-80 h-60 rounded-lg"></video>
    ...
    <div class="w-full md:w-80 bg-gray-50 rounded-lg p-4 shadow-sm">
        <ul class="space-y-4">
            <li>
                <p class="text-gray-500">Lesson Name:</p>
                <p class="text-xl font-semibold break-words">{{ $purchase->lesson->lesson_name }}</p>
            </li>
            <li>
            ...
            <li>
                ...
                <p class="text-xl font-semibold break-words">{{ $purchase->student->name }}</p>
            </li>
            <li>
            ...
            <li>
                ...
                <div class="rounded-pill px-4 py-2 w-auto text-white text-md font-bold inline-flex items-center gap-1 btn btn-success">
    ...
    <div class="feedback-sec border border-gray rounded-lg p-3 w-full md:w-96">
        <h2 class="font-bold text-2xl mb-3 border-b border-gray-500 pb-2">Feedback</h2>
        <div>
            ...
            @if($purchaseVideo->note ?? false)
                ...
                <p class="text-base font-semibold">{{ $purchaseVideo->note }}</p>
            @endif

            @if($purchaseVideo->feedback ?? false)
                <br>
                <p class="text-gray-500">{{ auth()->user()->type == 'Instructor' ? 'Your feedback' : 'Feedback' }}</p>
                <p class="text-base font-semibold">{{ $purchaseVideo->feedback }}</p>
            @endif
            ...
            <div class="flex flex-col md:flex-row items-start gap-3 mt-4">
                ...
                @else
                    <img class="w-20 h-14 object-cover cursor-pointer"
